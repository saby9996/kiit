...
class Wplms_EventOn_Init{
	...
	function event_display_setting($settings){
		$settings[]=array(
					'label'=>__('WPLMS Course Events Settings','vibe-customtypes' ),
					'type'=> 'heading',
				);
		$settings[] = array(
						'label' => __('Course Events calendar display','vibe-customtypes'),
						'name' =>'event_calendar_display',
						'class' => 'hide',
						'type' => 'select',
						'options'=>apply_filters('wplms_event_calendar_display',array(
							'' => __('Default, Daily Calendar','vibe-customtypes'),
							'list' => __('List Calendar','vibe-customtypes'),
							)),
						'desc' => __('Course - Events display','vibe-customtypes') 
					);
		return $settings;
	}
		...
		if(class_exists('WPLMS_tips')){
			...
			if(isset($tips->settings['event_calendar_display'])){
				switch($tips->settings['event_calendar_display']){
					case 'list':
						$shortcode = '[add_eventon wplms_course="1"]';
					break;
				}
			}
		}
