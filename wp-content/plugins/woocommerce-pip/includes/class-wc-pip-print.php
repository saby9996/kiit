	...
	public function print_document_action() {
		...
		if ( isset( $_GET['wc_pip_document'], $_GET['wc_pip_action'] ) && 'print' === $_GET['wc_pip_action'] ) {
			...
			if ( ! $can_admin && is_admin() ) {
				die( 'You are not allowed to view this page. You need to be an admin or be able to manage orders.' );
			}

			// get the requested document type and order(s)
			$type      = str_replace( '_', '-', $_GET['wc_pip_document'] );
			$order_id  = isset( $_GET['order_id'] )  ? (int) $_GET['order_id'] : 0;
			$order_ids = isset( $_GET['order_ids'] ) ? $_GET['order_ids']      : array();
			...
			if ( ! $order ) {
				// bail if no order
				die( 'The related order does not exist or is invalid.' );
			...
			if ( ! $document ) {
				...
				die( 'The requested document could not be found or is of an invalid type.' );
			}
