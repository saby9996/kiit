...
abstract class WC_PIP_Document {
	...
	public function __construct( $args ) {
		 ...
		 */
		$this->sort_items_by = apply_filters( 'wc_pip_document_sort_order_items_key', 'product', $this->order_id, $this->type );
	...
	public function get_customer_details() {
		 ...
		 * @param string $type WC_PIP_Document type
		 */
		return apply_filters( 'wc_pip_document_customer_details', $customer_details, $this->order_id, $this->type );
	}
	...
	public function get_invoice_number() {
		...
		if ( (int) $this->order_id > 0 && ! $invoice_number ) {
			...
			$invoice_number   = (int) get_option( 'wc_pip_invoice_number_start', '1' );
			...
			if ( $use_order_number ) {
				...
				// Bump the internal invoice number counter
				update_option( 'wc_pip_invoice_number_start', $invoice_number + 1 );
	...
	public function get_table_rows() {
		 ...
		 * @param string $document_type
		 */
		return apply_filters( 'wc_pip_document_table_rows', $table_rows, $items, $this->order->id, $this->type );
	}
	...
	protected function sort_order_items_by_column_key( $row_1, $row_2 ) {
		...
		$sort_order_items_key = $this->sort_items_by;
		...
		switch ( $sort_order_items_key ) {
			...
			case 'weight':
				...
				// TODO Use regex to strip any non-numerical character always instead {FN} 2016-04-20
				if ( 'price' === $sort_order_items_key ) {
					$item_1_value = str_replace( get_woocommerce_currency_symbol( $this->order->get_order_currency ), '', $item_1_value );
					$item_2_value = str_replace( get_woocommerce_currency_symbol( $this->order->get_order_currency ), '', $item_2_value );
				}
	...
	protected function get_table_row_order_item_data( $item_id, $item ) {
		 ...
		 * @param string $document_type
		 */
		return apply_filters( 'wc_pip_document_table_row_item_data', $item_data, $item, $product, $this->order_id, $this->type );
	}
	 ...
	 * @return string
	 */
	protected function get_order_item_sku_html( $product ) {
	...
	protected function get_order_item_name_html( $product, $item = '' ) {
		...
		if ( ! $product instanceof WC_Product ) {
			return '<span class="product">&ndash;</span>';
		}

		$product_name = '<span class="product ' . esc_attr( $this->get_order_item_product_classes( $product, $item ) ) . ' product-name">';
		$is_visible   = $product->is_visible();

		if ( $is_visible ) {
			$product_name .= sprintf( '<a href="%1$s" target="_blank">%2$s</a>', get_permalink( $product->id ), $product->get_title() );
		} else {
		 ...
		 */
		$product_name = apply_filters( 'wc_pip_order_item_name', $product_name, $item, $is_visible );
	...
	protected function get_order_item_meta_html( $item_id, $item, $product ) {
		...
		$item_meta = new WC_Order_Item_Meta( SV_WC_Plugin_Compatibility::is_wc_version_gte_2_4() ? $item : ( isset( $item['item_meta'] ) ? $item['item_meta'] : array() ) );

		$item_meta_html = '<div class="' . $this->get_order_item_product_classes( $product, $item ) . ' product-meta">';
		...
		if ( $this->order->has_status( array( 'completed', 'processing' ) ) && ( $purchase_note = get_post_meta( $product->id, '_purchase_note', true ) ) ) {
